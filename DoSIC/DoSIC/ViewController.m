...
//    hr = 1000.*((cv::getTickCount() - t_##name) / cv::getTickFrequency()))
#else
  #define TS(name)
  #define TE(name)
#endif
...
//    int64 t_name = cv::getTickCount();
    changeDetector->colorMagnify(image);
//    rate = 1000.0*(((double)cv::getTickCount() - (double) t_name) / (double) cv::getTickFrequency());
    hr = changeDetector->heartRate();
//    printf("TIMER_" ": %.2lfms\n", rate);
//    printf("heart rate" ": %4.1lf bpm\n", hr);
